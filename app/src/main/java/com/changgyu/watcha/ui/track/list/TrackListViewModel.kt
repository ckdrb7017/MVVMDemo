...
class TrackListViewModel @Inject constructor(
    ...
    private var _favoriteTrackItemList : MutableLiveData<List<FavoriteTrackEntity>> = MutableLiveData()
    ...
    fun insertFavoriteTrack(trackEntity: TrackEntity){
        viewModelScope.launch {
            ...
            if(result>0) showToast(WatchaTestApplication.watchaTestApp!!.applicationContext,"추가되었습니다.")
        }
    }
    ...
    fun deleteFavoriteTrackById(trackId: Int){
        viewModelScope.launch {
            ...
            if(result>0) showToast(WatchaTestApplication.watchaTestApp!!.applicationContext,"삭제되었습니다.")
        }
    }
    ...
    private fun convertTrackToFavoriteTrack(trackEntity: TrackEntity): FavoriteTrackEntity{
        return  FavoriteTrackEntity(
            trackId = trackEntity.trackId, collectionId = trackEntity.collectionId,
            artistId = trackEntity.artistId, trackName = trackEntity.trackName, collectionName = trackEntity.collectionName,
            artistName = trackEntity.artistName,artworkUrl = trackEntity.artworkUrl100, createdDate = getCurrentTime()
        )
    }
